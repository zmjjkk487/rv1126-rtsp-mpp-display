...
| 仓库 | 说明 |
|------|------|
| [camera-web-monitor-demo](https://github.com/zmjjkk487/camera-web-monitor-demo) | **生产交付版**：采集→编码→RTSP/RTP→WebRTC 全链路 + CMDP 私有发现协议 + 首登强制改密；含完整安全修复台账（22 项扫描：12 修 0 待 / 8 接受 / 3 低优先） |

本仓库为学习/展示原型（侧重代码注释与踩坑记录），生产与安全修复在关联仓库持续演进（`rtsp_server/` 由此拉出）。
