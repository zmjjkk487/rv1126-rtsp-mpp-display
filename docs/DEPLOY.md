...
| 云台 | 左转/右转/停止 (按住转, 松手停) + 预置位设置/调用/删除 |
| 状态 | 管线运行状态 |

### 6.1 云台控制 (ONVIF PTZ)

**目标 = 当前连接的摄像头** (config.ini), 标准 ONVIF ver20/ptz/wsdl:

| 操作 | 说明 |
|------|------|
| ContinuousMove | 方向控制, `tt:x` 速度 ±1; 前端按住转/松手停 |
| SetPreset / GotoPreset / RemovePreset | 预置位管理 (最多 8 个, token 自动分配 P1..P8) |
| GetPresets | 预置位列表 (web 面板 + ODM 工具可见) |

**本机无云台时的验证方式**: 摄像头接受指令后回调 `on_ptz_command` /
`on_ptz_preset` (producer.c) — 当前实现写 `/tmp/ptz_dir`, 显示端在
屏幕视频区中心画红色箭头 (方向) 或预置位标识 (如 "P1", 3 秒)。
接真实云台时只改这两个回调 (文件 → 电机控制), 协议层不变。

**关键实现约束** (踩坑记录):
- 屏幕标识必须画在 fbdev back 缓冲 (RGA 转换后、写显存前), **禁止写
  解码 DMABUF** — 会破坏 DMA 一致性 → rk_iommu 页错误 → 解码器挂死
- 箭头/标识叠加代码必须在 `write_fb:` 标签处 — RGA 成功路径会
